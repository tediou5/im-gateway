//! Once Cell impl borrowed from tokio.
...
use crate::semaphore::{Semaphore, SemaphorePermit, TryAcquireError};
...
impl<T> OnceCell<T> {
    ...
    fn set_value(&self, value: T, permit: SemaphorePermit<'_>) -> &T {
        ...
        *self.value_set.borrow_mut() = true;
    ...
    pub fn into_inner(mut self) -> Option<T> {
        if self.initialized_mut() {
            ...
            *self.value_set.get_mut() = false;
            ...
            let ptr = self.value.get();
            Some(unsafe { ptr::read(ptr).assume_init() })
        } else {
            None
        }
    }
