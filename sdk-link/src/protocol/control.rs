...
pub(crate) enum Event {
    Ack(u64),
    Package(u64, crate::protocol::content::Content),
    Disconnect(
        u64,
        bool,   /* stop reconnect */
        String, /* reason */
...
impl std::fmt::Debug for Event {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Ack(trace_id) => f.debug_struct("Ack").field("trace_id", trace_id).finish(),
            Self::Package(trace_id, _pack) => f
                .debug_struct("Package")
                .field("trace_id", trace_id)
                .finish(),
            Self::Disconnect(trace_id, stop_reconnect, reason) => f
                .debug_struct("Disconnect")
                .field("trace_id", trace_id)
                .field("stop_reconnect", stop_reconnect)
                .field("reason", reason)
                .finish(),
    ...
    fn decode(&mut self, src: &mut bytes::BytesMut) -> anyhow::Result<Option<Self::Item>> {
        ...
        let event = match (ack, weak_net_package) {
            (false, false) => {
                ...
                match (bad_network, heartbeat, number) {
                    (false, false, 0) => {
                        Event::Disconnect(trace_id, true, String::from_utf8_lossy(body).to_string())
                    }
                    (true, false, 0) => Event::Disconnect(
                        trace_id,
                        false,
                        String::from_utf8_lossy(body).to_string(),
                    ),
                    (_, _, number) if number != 1 => {
                        return Err(anyhow::anyhow!("PackageNumberMustBeOne: {number:?}"))
                    }
                    _ => Event::Package(trace_id, body.try_into()?),
                }
            }
            (true, false) => {
                if number != 1 {
